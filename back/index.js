...
let roomPlayers = {};
let salas = [
    { cantidad_participantes: 0, id_sala: 1, max_jugadores: 2, nombre_sala: "Sala_1" },
    { cantidad_participantes: 0, id_sala: 2, max_jugadores: 2, nombre_sala: "Sala_2" },
    { cantidad_participantes: 0, id_sala: 3, max_jugadores: 2, nombre_sala: "Sala_3" },
    { cantidad_participantes: 0, id_sala: 4, max_jugadores: 2, nombre_sala: "Sala_4" }
];
...
io.on("connection", (socket) => {
    ...
    socket.on('leaveRoom', async (data) => {
        ...
        const id_sala = await realizarQuery(`
            SELECT id_sala 
            FROM Salas 
    ...
    socket.on('joinRoom', async (data) => {
        ...
        // Sala llena
        if (salas[indiceSala].cantidad_participantes >= salas[indiceSala].max_jugadores) {
            socket.emit('maxPlayersReached', {
                message: "Se alcanzó el máximo de jugadores."
            });
            return;
        }
    ...
    //  ADIVINAR JUGADOR
    // =========================
    socket.on("adivinarJugador", async ({ room, userId, idJugadorAdivinado }) => {

        if (!roomPlayers[room]) return;
        ...
        const myGuess = idJugadorAdivinado;
        const opponentChoice = opponent.idJugadorSeleccionado;

        if (myGuess === opponentChoice) {

            io.to(socket.id).emit("ResultadoDeAcierto", { acierto: true });
            io.to(opponent.socketId).emit("ResultadoDeAcierto", { acierto: false });

            io.to(room).emit("finDelJuego", { ganador: userId });

            io.socketsLeave(room);

            delete roomPlayers[room];

            const indexSala = room - 1;
            salas[indexSala].cantidad_participantes = 0;

            await realizarQuery(`
                UPDATE Salas
                SET cantidad_participantes = 0
                WHERE id_sala = ${room}
            `);
            ...
            io.to(socket.id).emit("ResultadoDeAcierto", { acierto: false });
        }
    });
    ...
    // =========================
    socket.on("jugadorSeleccionado", ({ room, userId, idJugadorSeleccionado }) => {
